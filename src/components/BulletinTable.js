...
import "../styles/BulletinTable.css";
...
function BulletinTable() {
  ...
  const getData = async () => {
    try {
      ...
      setAllData(data);
      setSeperatedData(data.slice(0, PAGE_COUNT));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleTableDisplayCount = () => {
    ...
    setSeperatedData(allData.slice(0, loadCount));
  };
  ...
  return (
    <div>
      {seperatedData.length > 0 ? (
        <table className="bulletinTable" key={"header"}>
          <tbody>
            <tr>
              <th>Event Count: {seperatedData.length}</th>
              <th>Yorumlar</th>
              <th></th>
              <th>1</th>
              <th>x</th>
